...
elif exists("/boot/EFI/grub"):
    BOOT_GRUB_PATH = "/boot/EFI/grub"
else:
...
GRUB_MKCONFIG_PATH = which("grub-mkconfig-custom") or which("grub-mkconfig") or which("grub2-mkconfig")
...
def update_grub_cfg():
    ...
    update_command = (
        which("grub-mkconfig-custom") or which("update-grub") or which("grub-mkconfig") or which("grub2-mkconfig")
    )
    if update_command is None:
        error(
            f"Command for generating grub.cfg not found (tried update-grub, grub-mkconfig and grub2-mkconfig and grub-mkconfig-custom)"
        )
    command = f"{update_command} -o {GRUB_CFG_PATH} -g /boot/EFI/grub/grubenv"
